				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							this.uploadLists.splice(index, 1)
							this.$emit("inputlet", this.uploadLists);
							this.$emit("imgDelete", {
								del: delUrl,
						...
						const uploadTask = uni.uploadFile({
							...
							console.log(path)
								// if (typeof this.uploadSuccess == 'function') {
									 // console.log(this.uploadSuccess(uploadFileRes))
								...
								this.formData.host +
								"/" +
								this.formData.dir + "_"+ path
								})
								resolve(uploadFileRes);
									console.log(uploadFileRes,this.value)
									this.$emit("inputlet", this.value);
...
<style>
	...
	.htz-image-upload-Item-add>.cen{
		margin: 0 auto;
	}
	.htz-image-upload-Item-add>.cen>text{
		text-align: center;
	}
		.htz-image-upload-Item-add>.cen>text:nth-child(1){
			font-size: 66upx;
			display: block;
			/* font-weight: bold; */
				margin: 0 auto;
			...
			.htz-image-upload-Item-add>.cen>text:nth-child(2){
					margin: 0 auto;
				font-size: 26upx;
				margin-top: 20upx;
				display: block;
